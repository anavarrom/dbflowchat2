...
import dbflow.chat.config.KafkaProperties;
...
class Dbflowchat2KafkaResourceIT {
    ...
    void setup() {
        ...
        Dbflowchat2KafkaResource kafkaResource = new Dbflowchat2KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/dbflowchat-2-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/dbflowchat-2-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
